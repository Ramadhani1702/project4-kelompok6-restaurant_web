<%-- 
    Document   : Menu
    Created on : Jul 8, 2022, 1:47:36 PM
    Author     : Amalia Swastika
--%>

<%@page contentType="text/html" pageEncoding="UTF-8"%>
<!DOCTYPE html>
<html>

<head>
    <title>Menu</title>
    <link rel="shortcut icon" href="Image/Rajill.png">
    <link rel="stylesheet" href="CSS/Style.css">
    <link rel="stylesheet" href="CSS/MenuStyle.css">
    <link href="https://fonts.googleapis.com/css2?family=Josefin+Sans:ital,wght@0,100;0,200;0,300;0,400;0,500;0,600;0,700;1,100;1,200;1,300;1,400;1,500;1,600;1,700&display=swap" rel="stylesheet">
</head>

<body>


<!-- HEADER -->

<div class="header">
    <nav>
        <a href="#" class="logo"><img src="Image/Rajil.png"></a>
        <ul>
            <li class="active"><a href="RajilRestaurant.jsp">HOME</a></li>
            <li><a href="About.jsp">ABOUT</a></li>
            <li class="active"><a href="Menu.jsp">MENU</a></li>
            <li><a href="Outlets.jsp">OUTLETS</a></li>
            <li><a href="Promo.jsp">PROMO</a></li>
        </ul>
        <a href="Login.jsp" class="btn">RESERVATION</a>
    </nav>
</div>


<!-- CONTENT -->

<div class="content">
    <h1 id="opening">WELCOME AT RAJIL RESTAURANT!</h1>
    <h3 id="tagline">Special Menu For Special People.</h3>
</div>


<!-- GET KNOW US -->

<div>
    <ul class="sci">
        <li>
            <a href="https://wa.me/6282112400042"><img src="Image/Whatsapp.png"></a>
        </li>

        <li>
            <a href="https://instagram.com/rajilrestaurant?igshid=YmMyMTA2M2Y="><img src="Image/Instagram.png"></a>
        </li>

        <li>
            <a href="https://twitter.com/rajilrestaurant?s=21&t=8iWkeYv6NyW9M3yOYkgfpw"><img src="Image/Twitter.png"></a>
        </li>
    </ul>
</div>
</div>


<!-- MENU -->

<div class="row">
    <div class="col-12 text-center menu-title">
        <h1>MENU</h1>
    </div>

    <div class="col-12 menu-title">
        <div class="row">
            <div class="col-6 card-padding">
                <div class="card-menu">
                    <div class="row">
                        <div class="col-6">
                            <img class="card-image" src="Image/Rama-Shinta.jpg" alt="Rama Icon">
                        </div>

                        <div class="col-6">
                            <div class="row text-left">
                                <div class="col-12">
                                    <h1>RAMA & SHINTA</h1>
                                </div>
                            </div>

                            <div class="row text-left">
                                <div class="col-6">
                                    <p>Menu</p>
                                </div>

                                <div class="col-6">
                                    <p>Burger & Drink</p>
                                </div>

                                <div class="col-6">
                                    <p>Price</p>
                                </div>

                                <div class="col-6">
                                    <p>Rp400.000</p>
                                </div>

                                <div class="col-6">
                                    <p>For</p>
                                </div>

                                <div class="col-6">
                                    <p>2 People</p>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
            
            <div class="col-6 card-padding">
                <div class="card-menu">
                    <div class="row">
                        <div class="col-6">
                            <img class="card-image" src="Image/Three-Muskeeter.jpg" alt="Three Icon">
                        </div>
                        
                        <div class="col-6">
                            <div class="row text-left">
                                <div class="col-12">
                                    <h1>THREE MUSKEETER</h1>
                                </div>
                            </div>
                            
                            <div class="row text-left">
                                <div class="col-6">
                                    <p>Menu</p>
                                </div>
                                
                                <div class="col-6">
                                    <p>Steak & Drink</p>
                                </div>

                                <div class="col-6">
                                    <p>Price</p>
                                </div>

                                <div class="col-6">
                                    <p>Rp600.000</p>
                                </div>
                                
                                <div class="col-6">
                                    <p>For</p>
                                </div>

                                <div class="col-6">
                                    <p>3 People</p>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
        
        <div class="row">
            <div class="col-6 card-padding">
                <div class="card-menu">
                    <div class="row">
                        <div class="col-6">
                            <img class="card-image" src="Image/Qura-Qura-Ninja.jpg" alt="Qura Icon">
                        </div>
                        
                        <div class="col-6">
                            <div class="row text-left">
                                <div class="col-12">
                                    <h1>QURA QURA NINJA</h1>
                                </div>
                            </div>
                            
                            <div class="row text-left">
                                <div class="col-6">
                                    <p>Menu</p>
                                </div>

                                <div class="col-6">
                                    <p>Pizza & Drink</p>
                                </div>

                                <div class="col-6">
                                    <p>Price</p>
                                </div>

                                <div class="col-6">
                                    <p>Rp800.000</p>
                                </div>
                                
                                <div class="col-6">
                                    <p>For</p>
                                </div>

                                <div class="col-6">
                                    <p>4 People</p>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
            
            <div class="col-6 card-padding">
                <div class="card-menu">
                    <div class="row">
                        <div class="col-6">
                            <img class="card-image" src="Image/Acelole.jpg" alt="Acelole Icon">
                        </div>
                        
                        <div class="col-6">
                            <div class="row text-left">
                                <div class="col-12">
                                    <h1>ACELOLE</h1>
                                </div>
                            </div>
                            
                            <div class="row text-left">
                                <div class="col-6">
                                    <p>Menu</p>
                                </div>
                                
                                <div class="col-6">
                                    <p>Barbeque & Drink</p>
                                </div>

                                <div class="col-6">
                                    <p>Price</p>
                                </div>

                                <div class="col-6">
                                    <p>Rp1.000.000</p>
                                </div>
                                
                                <div class="col-6">
                                    <p>For</p>
                                </div>

                                <div class="col-6">
                                    <p>5 - 7 People</p>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
</div>


<!-- FOOTER -->

<footer>
    <div class="container">
        <small>Copyright &copy; 2022 - RAJIL RESTAURANT. All Rights Reserved.</small>
    </div>
</footer>

    </body>
</html>
